...
def process_answer_with_citations(answer, context_docs):
    ...
    # Add citations after key statements, numbers, or facts
    processed_answer = answer
    
    # Add citations after sentences that contain specific data or references
    patterns_to_cite = [
        r'(Item[‑\-]\s*No\.\s*[\d\-‑]+[\w\-‑]*)',  # Item numbers like "Item-No. 28-43-1"
        r'(Rs\.?\s*[\d,]+\.?\d*)',  # Monetary amounts like "Rs 1,838.70"
        r'(Chapter\s*[\d]+)',  # Chapter references like "Chapter 28"
        r'(\d+\.?\d*\s*%)',  # Percentages like "20%"
        r'(Rule\s*[\d\.]+)',  # Rule numbers like "Rule 25.4.18"
        r'(\d+″\s*×\s*\d+″)',  # Dimensions like '2" × 2"'
        r'(Sq\s*[mft]+)',  # Units like "Sq m", "Sq ft"
        r'(MRS|CSR|Market[‑\-]Rate\s*System)',  # Acronyms
    ]
    ...
    for pattern in patterns_to_cite:
        ...
        for match in reversed(matches):  # Reverse to maintain positions
            # Find the best matching citation based on content similarity
            ...
            if best_citation_idx is not None and best_score > 0:
                ...
                # Insert citation after the match
                end_pos = match.end()
                processed_answer = (processed_answer[:end_pos] + 
                                  citation_link + 
                                  processed_answer[end_pos:])
    
    # Add any unused citations at the end of major paragraphs
    unused_citations = [citations[f"[{i+1}]"] for i, used in enumerate(citation_usage) if not used]
    if unused_citations:
        # Add remaining citations after the last period of major paragraphs
        paragraphs = processed_answer.split('\n\n')
        if len(paragraphs) > 1:
            # Add citations to the end of the first major paragraph
            paragraphs[0] += ' ' + ''.join(unused_citations[:2]) if len(unused_citations) >= 2 else ''.join(unused_citations)
            processed_answer = '\n\n'.join(paragraphs)
...
<style>
    ...
    .answer-content h1, .answer-content h2, .answer-content h3 { color: #4CAF50; margin-top: 1.5rem; margin-bottom: 0.8rem; }
    .answer-content table { border-collapse: collapse; width: 100%; margin: 1rem 0; }
    .answer-content th, .answer-content td { border: 1px solid #ddd; padding: 8px 12px; text-align: left; }
    .answer-content th { background-color: #f8f9fa; font-weight: bold; color: #4CAF50; }
    .answer-content ul, .answer-content ol { margin: 1rem 0; padding-left: 2rem; }
    .answer-content li { margin: 0.5rem 0; line-height: 1.6; }
</style>
...
# Chat interface screen
    ...
    Be comprehensive, detailed, and include specific references to:
    - Item numbers (e.g., Item-No. 28-43-1)
    - Chapter references (e.g., Chapter 28)
    - Specific rates and amounts (e.g., Rs 1,838.70)
    - Rules and regulations (e.g., Rule 25.4.18)
    - Technical specifications and measurements
    - Document names when relevant

    Format your response with clear structure using headers, bullet points, and tables where appropriate.
            ...
            with st.spinner("🤖 AI Processing your question..."):
                try:
                    ...
                    with st.expander("📚 Sources Used", expanded=False):
                        ...
                        # Create a summary table of sources
                        ...
                        for i, doc in enumerate(response["context"]):
                            ...
                            source_data.append({
                                ...
                                "Page": page_num,
                                "Available": "✅ Yes" if (full_path and os.path.exists(full_path)) else "❌ No"
                            })
                        ...
                        for i, doc in enumerate(response["context"]):
                            ...
                            with st.container():
                                ...
                                with col2:
                                    if full_path and os.path.exists(full_path):
                                        st.markdown(f"""
                                        <a href="file:///{full_path}" target="_blank" style="background: #4CAF50; color: white; padding: 8px 16px; border-radius: 12px; text-decoration: none; font-weight: bold; display: inline-block;">
                                            🔗 Open Document
                                        </a>
                                        """, unsafe_allow_html=True)
                                    else:
                                        st.markdown("🚫 File not accessible")
                                
                                # Show content preview
                                content_preview = doc.page_content[:400] + "..." if len(doc.page_content) > 400 else doc.page_content
                                st.markdown(f"**📝 Content Preview:**")
                                st.markdown(f'<div style="background: #f8f9fa; padding: 1rem; border-radius: 8px; border-left: 4px solid #4CAF50; font-style: italic; color: #555;">{content_preview}</div>', unsafe_allow_html=True)
                                
                                if i < len(response['context']) - 1:
                                    st.divider()
                    
                    # Add assistant message with citation count
                    citation_count = len(response['context'])
                    enhanced_answer = f"{answer}\n\n*📊 Citations: {citation_count} sources referenced*"
